...
async function init() {
    ...
    function compute_jitters(subdivisionLevel) {
        const jitters = [];
        const step = 1 / subdivisionLevel;
        for (let y = 0; y < subdivisionLevel; y++) {
            for (let x = 0; x < subdivisionLevel; x++) {
                const jitterX = (x + Math.random()) * step - 0.5;
                const jitterY = (y + Math.random()) * step - 0.5;
                jitters.push(jitterX, jitterY);
            }
        }
        return new Float32Array(jitters);
    }

    let subdivisionLevel = 1;
    let maxSubdivisionLevel = 5;
    let jittersBuffer;

    function initJittersBuffer() {
        const maxJittersSize = compute_jitters(maxSubdivisionLevel).byteLength;
        jittersBuffer = device.createBuffer({
            size: maxJittersSize,
    ...
    function updateJitters() {
        const jitters = compute_jitters(subdivisionLevel);
        device.queue.writeBuffer(jittersBuffer, 0, jitters);
    }
    ...
    subdivisionLevelInput.addEventListener('change', function() {
        subdivisionLevel = Math.min(parseInt(this.value), maxSubdivisionLevel);
        updateJitters();
        updateUniforms();
    });
    ...
    const shader = `
        ...
        @group(0) @binding(4) var samplerRepeat: sampler;
        @group(0) @binding(5) var<storage, read> jitters: array<vec2<f32>>;
        ...
        fn sampleTexture(uv: vec2<f32>) -> vec3<f32> {
            ...
            if (uniforms.useTexture < 1.5) {
                return textureSample(textureImage, samplerNearest, uv).rgb;
            } else if (uniforms.useTexture < 2.5) {
                return textureSample(textureImage, samplerLinear, uv).rgb;
            } else {
                return textureSample(textureImage, samplerRepeat, repeatUV).rgb;
            }
        }
        ...
        fn intersectPlane(ray: Ray, position: vec3<f32>, normal: vec3<f32>) -> HitInfo {
            ...
            if (hit.hit) {
                ...
                let xAxis = normalize(cross(normal, vec3<f32>(0.0, 0.0, 1.0)));
                let zAxis = cross(normal, xAxis);
                hit.uv = vec2<f32>(
                    dot(hit.position - position, xAxis),
                    dot(hit.position - position, zAxis)
                ) * 5.0;
            }
        ...
        fn shade(hit: HitInfo, ray: Ray) -> vec3<f32> {
            ...
            for (var depth = 0; depth < MAX_DEPTH; depth++) {
                ...
                if (!currentHit.isMirror && !currentHit.isRefractive && !currentHit.isGlossy) {
                    break;
                } else if (currentHit.isMirror) {
        ...
        fn fragmentMain(@location(0) uv: vec2<f32>) -> @location(0) vec4<f32> {
            var finalColor = vec3<f32>(0.0);
            let subPixelCount = u32(uniforms.subdivisionLevel * uniforms.subdivisionLevel);

            for (var i: u32 = 0u; i < subPixelCount; i++) {
                let jitter = jitters[i];
                let subPixelUV = uv + jitter / vec2<f32>(f32(textureDimensions(textureImage).x), f32(textureDimensions(textureImage).y));
                let ray = generateRay(subPixelUV);
                let hitInfo = intersectScene(ray);

                var textureColor = vec3<f32>(1.0, 1.0, 1.0);
                let sampleUV = fract(hitInfo.uv * uniforms.uv_scale);


                var nearestSample = textureSample(textureImage, samplerNearest, sampleUV).rgb;
                var linearSample = textureSample(textureImage, samplerLinear, sampleUV).rgb;
                var repeatSample = textureSample(textureImage, samplerRepeat, sampleUV).rgb;


                if (uniforms.useTexture > 2.5) {
                    textureColor = repeatSample;
                } else if (uniforms.useTexture > 1.5) {
                    textureColor = linearSample;
                } else if (uniforms.useTexture > 0.5) {
                    textureColor = nearestSample;
                }

                var subPixelColor: vec3<f32>;
                if (hitInfo.hit) {
                    var shadedColor = shade(hitInfo, ray);
                    if (!hitInfo.isMirror && !hitInfo.isRefractive && !hitInfo.isGlossy) {
                        shadedColor *= textureColor;
                    }
                    subPixelColor = shadedColor;
                } else {
                    subPixelColor = vec3<f32>(0.1, 0.3, 0.6);
                }

                finalColor += subPixelColor;
            }

            finalColor /= f32(subPixelCount);
    ...
    const bindGroup = device.createBindGroup({
        ...
        entries: [
            ...
            {
                binding: 2,
                resource: samplerNearest,
            },
            {
                binding: 3,
                resource: samplerLinear,
            },
            {
                binding: 4,
    ...
    textureSelect.addEventListener('change', function() {
        useTexture = parseInt(this.value);
        updateUniforms();
    });
    ...
    sphereMaterialSelect.innerHTML = `
        ...
        <option value="1" selected>Mirror</option>
        <option value="2">Phong</option>
        <option value="3">Refractive</option>
        <option value="4">Glossy</option>
    `;
