async function init() {
    ...
    const shader = `
        ...
        fn createCoordinateSystem(normal: vec3<f32>) -> mat3x3<f32> {
            let a = select(vec3<f32>(0.0, 1.0, 0.0), vec3<f32>(1.0, 0.0, 0.0),
                        abs(normal.x) > 0.9);
            let t = normalize(cross(a, normal));
            let b = cross(normal, t);
            return mat3x3<f32>(t, b, normal);
        }

        fn sampleCosineHemisphere(normal: vec3<f32>, seed: vec2<f32>) -> vec3<f32> {
            let r1 = rand(seed);
            let r2 = rand(seed + vec2<f32>(1.0, 1.0));
            ...
            let coords = createCoordinateSystem(normal);
            return normalize(coords * vec3<f32>(x, y, z));
        }
        ...
        fn computeRadiance(ray: Ray, seed: vec2<f32>) -> vec3<f32> {
            ...
            for (var depth = 0; depth < 5; depth++) {
                ...
                for (var i: u32 = 0u; i < arrayLength(&lightIndices); i++) {
                    directLight += sampleAreaLight(hitInfo.position, i,
                        currentSeed + vec2<f32>(f32(depth) * 2.0, f32(i) * 2.0));
                }
                result += throughput * color * directLight;


                let bounceDir = sampleCosineHemisphere(hitInfo.normal,
                    currentSeed + vec2<f32>(f32(depth) * 3.0, 0.0));


                throughput *= color * 0.8;


                let p = max(throughput.r, max(throughput.g, throughput.b));
                if (rand(currentSeed + vec2<f32>(f32(depth) * 4.0, 0.0)) > p) {
                    break;
                }
                throughput /= p;


                currentRay.origin = hitInfo.position + hitInfo.normal * 0.001;
                currentRay.direction = bounceDir;
                currentRay.tmin = 0.001;
                currentRay.tmax = 1000.0;


                currentSeed += vec2<f32>(f32(depth) * 5.0, f32(depth) * 5.0);
            }
        ...
        fn fragmentMain(
            ...
            let color = computeRadiance(ray, seed);

            let newColor = vec4<f32>(color, 1.0);
