async function init() {
    ...
    const shader = `
        ...
        fn sampleDirectionalLight() -> Light {
            ...
            light.direction = normalize(vec3<f32>(1.0, 0.2, 0.0));

            light.color = vec3<f32>(5.0, 4.8, 4.5);
            return light;
        }
            ...
            if (hitInfo.hit) {
                switch(shadingMode) {
                    ...
                    case 2u: {
                        ...
                        let ambient = sampleEnvironmentMap(hitInfo.normal).xyz * 0.2;

                        let directLight = diffuse * light.color * lambert;
                        color = vec4<f32>(directLight + ambient, 1.0);
                    }
                    default: {
    ...
    envMapImage.src = 'luxo-pxr-campus.jpg';
    ...
    device.queue.writeBuffer(shadingModeBuffer, 0, new Uint32Array([0]));
