async function init() {
    ...
    const shader = `
        ...
        fn shade(hit: HitInfo, ray: Ray) -> vec3<f32> {
            ...
            var currentRay = ray;
            var currentHit = hit;
            var reflectionFactor = 1.0;

            for (var depth = 0; depth < MAX_DEPTH; depth++) {
                if (currentHit.isMirror && uniforms.sphereMaterial == 1.0 && depth < MAX_DEPTH - 1) {
                    ...
                    let nextHit = intersectScene(currentRay);
                    if (!nextHit.hit) {
                        finalColor += reflectionFactor * vec3<f32>(0.1, 0.3, 0.6);
                        break;
                    }
                    ...
                    if (currentHit.isMirror) {
                        ...
                        if (uniforms.sphereMaterial == 0.0 || uniforms.sphereMaterial == 2.0) {
                            ...
                            if (uniforms.sphereMaterial == 2.0) {

                                finalColor += reflectionFactor * currentHit.color;
                            } else if (!shadowHit.hit) {
                                ...
                                let diffuseColor = 0.9 * currentHit.color * diffuse * attenuation * light.intensity;
                                finalColor += reflectionFactor * (ambient + diffuseColor);
                            } else {
                                finalColor += reflectionFactor * ambient;
                            }
                        }
                    } else {

                        if (uniforms.matteMaterial == 0.0 || uniforms.matteMaterial == 1.0) {
                            let light = samplePointLight(currentHit.position);
                            let lightDir = normalize(light.position - currentHit.position);
    ...
    function updateUniforms() {
        const uniformData = new Float32Array([aspectRatio, cameraConstant, sphereMaterial, matteMaterial]);
