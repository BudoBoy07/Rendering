...
// Also add this if it's used in your code
function vec4(v, w = 1.0) {
  if (Array.isArray(v) || v instanceof Float32Array) {
      return new Float32Array([v[0], v[1], v[2], w]);
  }
  return new Float32Array([v, v, v, w]);
}
...
function flatten(arr) {
  ...
  for (let i = 0; i < arr.length; i++) {
      result[i * 4] = arr[i][0];
      result[i * 4 + 1] = arr[i][1];
      result[i * 4 + 2] = arr[i][2];
      result[i * 4 + 3] = arr[i][3];
  }
  return result;
}
