async function init() {
    ...
    const shader = `
        ...
        fn fresnel_R(cos_theta_i: f32, cos_theta_t: f32, eta_i_over_t: f32) -> f32 {
            ...
            let r_perp = (eta_i_over_t * cos_theta_i - cos_theta_t) /
                        (eta_i_over_t * cos_theta_i + cos_theta_t);

            let r_para = (cos_theta_i - eta_i_over_t * cos_theta_t) /
                        (cos_theta_i + eta_i_over_t * cos_theta_t);
        ...
        fn intersectScene(ray: Ray) -> HitInfo {
            ...
            let leftSphereHit = intersectSphere(ray, vec3<f32>(420.0, 90.0, 370.0), 90.0);
            if (leftSphereHit.hit && leftSphereHit.distance < hitInfo.distance) {
                hitInfo = leftSphereHit;
                hitInfo.materialIndex = 998u;
            }


            let rightSphereHit = intersectSphere(ray, vec3<f32>(130.0, 90.0, 250.0), 90.0);
            if (rightSphereHit.hit && rightSphereHit.distance < hitInfo.distance) {
                hitInfo = rightSphereHit;
                hitInfo.materialIndex = 999u;

                hitInfo.extinction = vec3<f32>(0.2, 0.4, 0.8);
            }
        ...
        fn traceRay(ray: Ray) -> vec3<f32> {
            ...
            for (var bounce = 0; bounce < 8; bounce++) {
                let hitInfo = intersectScene(currentRay);
                ...
                if (hitInfo.materialIndex == 999u) {
                    let eta = 1.5;
                    let eta_ratio = select(1.0/eta, eta, currentRay.isInside);

                    let cos_theta_i = abs(dot(-currentRay.direction, hitInfo.normal));
                    let sin2_theta_t = eta_ratio * eta_ratio * (1.0 - cos_theta_i * cos_theta_i);
                    let cos_theta_t = sqrt(1.0 - sin2_theta_t);


                    let R = fresnel_R(cos_theta_i, cos_theta_t, 1.0/eta_ratio);


                    let pathLength = hitInfo.distance;

                    if (currentRay.isInside) {

                        let transmittance = exp(-hitInfo.extinction * pathLength);

                        let avgTransmittance = (transmittance.r + transmittance.g + transmittance.b) / 3.0;

                        let random = rand(vec2<f32>(currentRay.origin.xy) + uniforms.frameCount);

                        if (random > avgTransmittance) {

                            break;
                        } else {

                            throughput *= transmittance / avgTransmittance;
                        }
                    }
                    ...
                    if (random < R) {
                        ...
                        currentRay.direction = refract(currentRay.direction, hitInfo.normal, eta_ratio);
                        currentRay.isInside = !currentRay.isInside;
                    }
                    currentRay.origin = hitInfo.position + currentRay.direction * 0.001;
                    throughput *= vec3<f32>(0.98);

                    finalColor += vec3<f32>(0.5);

                } else if (hitInfo.materialIndex == 998u) {
                    currentRay.direction = reflect(currentRay.direction, hitInfo.normal);
                    currentRay.origin = hitInfo.position + currentRay.direction * 0.001;
                    throughput *= vec3<f32>(0.9);
                    ...
                    for (var i = 0u; i < arrayLength(&lightIndices); i++) {
                        directLight += sampleAreaLight(hitInfo.position, i,
                            vec2<f32>(currentRay.origin.xy) + uniforms.frameCount + vec2<f32>(f32(i) * 2.0, f32(i) * 2.0));
                    }
        ...
        fn fragmentMain(
            ...
            var pixelPos = vec2<f32>(
                fragPos.x + getRandom2D(vec2<f32>(fragPos.xy) + uniforms.frameCount).x,
                fragPos.y + getRandom2D(vec2<f32>(fragPos.xy) + uniforms.frameCount).y
            );
            ...
            if (uniforms.frameCount > 0.0) {
                ...
                return mix(prevColor, vec4<f32>(color, 1.0), 1.0 / (uniforms.frameCount + 1.0));
            }
